...
class SimpleMsSqlConnection():
    def __init__(self, Database = ""):
        ...
        self.port = os.getenv('AWS_DEV_DB_PORT')
        self.ConnectionString = "host={0} user={1} dbname={2} password={3} sslmode={4} port={5}".format(self.host, self.user,
        Database, self.password, self.sslmode, self.port)
...
def insertContributorsToDb(sqlParameters = ''):
    ...
    connection = SimpleMsSqlConnection(Database=os.getenv('AWS_COLLECTION_DEV_DB_NAME'))
    connection.runSQL(query, sqlParameters)

def main():
    sqlParameters = generateInsertSqlParameters(rowsToInsert=100, startReference='77700000001', survey='066',
                                                period='201512')
